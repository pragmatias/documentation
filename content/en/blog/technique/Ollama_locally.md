...
title : "LLMs : Using Ollama with Podman and VSCode"
date : 2024-07-24
draft : false
toc: true
---
...
[Podman](https://docs.podman.io/en/latest/) is a daemonless, open source, Linux native tool designed to make it easy to find, run, build, share and deploy applications using Open Containers Initiative (OCI) Containers and Container Images. 
...
Recommended : You will need to use the [GPU Passthrough](https://docs.nvidia.com/ai-enterprise/deployment-guide-rhel-with-kvm/0.1.0/podman.html) to have a good performance


# VS Code

[VS Code](https://code.visualstudio.com/) is a multi-platform source-code editor developed by Microsoft. 
VS Code can be extended via extensions available through a central repository.

[Continue](https://www.continue.dev/) is an open source AI code assistant which allow to connect any models inside the IDE.

We will use the VS Code Continue extension to work with your Ollama configuration (local or remote). Additionally, you can leverage various providers and services, such as [Open AI](https://openai.com/), [Anthropic](https://www.anthropic.com/), [Mistral](https://mistral.ai/), [Gemini](https://gemini.google.com/) and others.
...
[![continue_step_1](/blog/web/20240724_ollama_locally_p1.png)](/blog/web/20230724_ollama_locally_p1.png) 

4. Install the extension named **Continue - Codestral, Claude, and more**
5. Select the **Continue** icon in the left panel 

[![continue_step_2](/blog/web/20240724_ollama_locally_p2.png)](/blog/web/20230724_ollama_locally_p2.png) 


To configure the Continue extension in VSCode :
You can access at the `config.json` file in two different ways.

1. VS Code interface : 
    1. Go to VS Code
    2. Click on the **Continue** menu in the left panel
    3. Click on the **Configure Continue** on the bottom right of the new left panel

[![continue_step_3](/blog/web/20240724_ollama_locally_p3.png)](/blog/web/20230724_ollama_locally_p3.png) 

2. VS Code command :
    1. Go to VS Code
    2. Open the command Palette with the combination `Ctrl + Shift + p`
    3. Use the search term `continue option` and click on the `Continue : open config.json` option

[![continue_step_4](/blog/web/20240724_ollama_locally_p4.png)](/blog/web/20230724_ollama_locally_p4.png) 
...
To install and use the Ollama tool : 
...
Result: 
[![ollama_local_step_1](/blog/web/20240724_ollama_locally_p5.png)](/blog/web/20230724_ollama_locally_p5.png) 


## VS Code Continue extension configuration 

Example of a local Ollama configuration for VS Code continue extension :
```JSON
{
  "models": [
    {
...
## Ollama remote installation (with Podman)
...
2. Create a Container : `podman run -dt --pod llms-pod --name llms-pod-ollama docker.io/ollama/ollama:latest` 
3. Pull models : `podman exec -it llms-pod-ollama ollama pull llama3:8b` 
4. Run a specific models : `podman exec -it llms-pod-ollama ollama run llama3:8b` ` 

You will find some scripts to manage the podman pod more easily in this [github repo](https://github.com/pragmatias/ollama_local)

Result :
[![ollama_local_step_2](/blog/web/20240724_ollama_locally_p6.png)](/blog/web/20230724_ollama_locally_p6.png) 


## Continue extension configuration

Example of a remote Ollama configuration for VSCode continue extension :
```JSON
{
  "models": [
    {
...
It's very easy to start using LLMs locally for privacy purpose with Ollama on all platform (Linux, Windows or Mac) and VS Code. 
